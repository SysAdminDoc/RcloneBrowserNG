...
### Reliability
-   NEW: Copy, move, sync and bisync now run as native remote-control calls instead of the command line being run inside the daemon. Each job gets `sync/copy` and friends with its own stats group, which is what live bandwidth tuning and a graceful stop need to attach to; the job card reads its progress from that group. Anything that cannot be expressed as an API call still runs the old way, and that judgement is not guesswork: the flag-to-option table is read from the daemon's own `options/info`, so a flag this rclone does not know refuses the whole request rather than being dropped. That matters because rclone accepts an unknown option key with a 200 and ignores it, which would otherwise let a mistyped `--dry-run` disappear in silence. A single file as the source, a delete or a purge, and an rclone too old for the endpoint all keep the old route too
...
-   NEW: The exclude editor now says what each pattern will actually hit. rclone filters are unanchored, so `*.tmp` typed as if it meant one folder quietly applies everywhere, and a line under the editor now spells that out per pattern with a concrete example of what it matches and, for an anchored pattern, what it leaves alone. Ticking "delete excluded" restyles the whole panel as a warning and says the matches are removed at the destination rather than skipped. That combination is what cost an upstream reporter data (kapitainsky#252)
...
-   FIX: "Copy on the server where possible" now actually adds the flag. The option was read back with `isVisible()`, which is false for a child of a hidden window and false for a control on a tab that is not the current one. `QDialog::exec()` hides the dialog before it returns, so every interactive transfer read the tick as unticked and `--server-side-across-configs` never reached the command line; a saved task also came out different depending on which tab was open when it was saved. Ticking it and then retargeting the transfer still drops it, which is the behaviour that was intended all along
-   FIX: The exclude panel no longer promises that an anchored `**` pattern stays at the top level. `/**` matches every file in the transfer and `/**.tmp` matches at every depth, but the panel answered both with "only at the top level ... but not sub/...", printed in the warning box beside "delete excluded". A pattern rclone will reject outright, like `a{b`, now says so instead of describing a transfer that cannot start
-   FIX: A cancelled transfer no longer appears in the job history as an rclone exit code. Cancelling ends in a kill, and the number that comes back is the kill's: 62097 on Windows, signal 9 on Linux, which the exit-code table reads as "Nothing to transfer" and counts as a success. The card and the history now take their wording from one place, so they cannot disagree about the same run
-   FIX: Pruning old backup directories and post-transfer verification now require a complete copy, not merely an acceptable one. Both were gated on the same flag that treats `--max-transfer` and `--max-duration` as success, so a run cut short at a limit could `rclone purge` its own backups and then check a copy it knew was partial
-   FIX: The transfer dialog no longer freezes for up to five seconds on the first keystroke in a path field, and no longer hides the server-side option for the rest of the session when the first remote-type lookup fails. The lookup ran `rclone listremotes` synchronously on the window thread and marked itself done before the process had run, so one slow or password-locked config poisoned the cache permanently
