...
class RcAuthRegressionTest : public QObject {
  Q_OBJECT

private slots:
  void rcEndpointsRequireAuth() {
    ...
    QVERIFY2(!sites.isEmpty(), "Found zero RC command sites -- test is broken");
    ...
    for (const auto &site : sites) {
      ...
      if (site.text.contains("\"rcd\"")) {
        ...
        QVERIFY2(foundAuth,
                 qPrintable(
                     QString("rcd startup at %1:%2 has no --rc-user/--rc-pass "
                             "within +/-15 lines")
                         .arg(site.file)
                         .arg(site.line)));
      }
      ...
      if (site.text.contains("--rc-addr")) {
        ...
        QVERIFY2(foundAuth,
                 qPrintable(
                     QString("RC endpoint at %1:%2 has no auth credentials "
                             "within +/-15 lines")
                         .arg(site.file)
                         .arg(site.line)));
      }
    ...
    QVERIFY2(!hasRcNoAuth,
             qPrintable("SECURITY: --rc-no-auth found in source:\n  " +
                         noAuthViolations.join("\n  ")));
    QVERIFY2(hasRcdStartup, "Expected at least one rcd startup site");
    QVERIFY2(hasMountRc, "Expected at least one --rc-addr site");
  }
};

QTEST_MAIN(RcAuthRegressionTest)
#include "rc_auth_regression_test.moc"
