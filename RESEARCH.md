...
- **GUI-thread blocking on user actions.** `schedule_manager.cpp` makes seventeen `waitForFinished` calls with 3s-15s timeouts against `schtasks`/`launchctl`/`systemctl`, reached from the Schedule dialog's Save and Delete actions. `remote_widget.cpp:210` runs a nested `QEventLoop` with a 30-second timeout inside `remoteFingerprint()`, on the Open/Edit context-menu path. `rclone_rc_engine.cpp:55-71` calls `waitForStarted(5000)` and then spins on `QThread::msleep(50)` with `processEvents`, on the click that starts an RC-backed transfer.
