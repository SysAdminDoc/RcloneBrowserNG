...
## P1 — Distribution (needs a macOS build host)
...
- [ ] P1 — Publish a maintainer-owned Homebrew tap
  ...
  Complexity: S
  Blocked (2026-09-05): there is no macOS artifact to point a cask at, and none can be produced from this machine. The v2.0.2 release carries only the Windows installer, the Windows zip and a source tarball; `scripts/release_macOS.sh` requires macOS with Xcode command line tools, Homebrew and `qt@6`. Both halves of the acceptance depend on that artifact existing: the cask has to install it and its sha256 has to match the published asset.
  Unblocks when: someone runs `scripts/release_macOS.sh` on a Mac and the resulting DMG is attached to a GitHub release. The cask itself is already generated by `scripts/generate_package_manifests.py`, and `scripts/release_check.py` writes the SHA256SUMS, so the remaining work is creating the `SysAdminDoc/homebrew-rclonebrowserng` repository and committing the generated cask.
